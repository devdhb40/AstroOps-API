...
import { ApodListResponseDto } from './dtos/response.dto';
...
export class ApodController {
  ...
  @BaseResponse(ApodListResponseDto)
  @ApiQuery({ type: ApodQueryDto })
  async getApod(@Query() query: ApodQueryDto) {
    const result = await this.apodService.getApod(query);
    return {
      data: result.items.map((item) => ApodMapper.toDto(item)),
      meta: {
